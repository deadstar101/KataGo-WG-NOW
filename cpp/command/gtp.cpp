  ...
  string rawNN(int whichSymmetry, double policyOptimism, bool useHumanModel, Player nextPla) {
    ...
    for(int symmetry = 0; symmetry < SymmetryHelpers::NUM_SYMMETRIES; symmetry++) {
      if(whichSymmetry == NNInputs::SYMMETRY_ALL || whichSymmetry == symmetry) {
        ...
        // WG Phase-3C TEMPORARY scalar visibility gate.
        // Outside buf.hasWeakGroupMap so stock/D0 also reports has=0.
        out << "WG_SCALAR has=" << (nnOutput->hasWeakGroupDanger ? 1 : 0)
            << " current=" << nnOutput->weakGroupCurrentDanger
            << " opponent=" << nnOutput->weakGroupOpponentDanger
            << endl;

        // WG Phase-3C FINAL cache/multisym diagnostic.
        // Run once when rawNN is explicitly evaluating all 8 symmetries.
        if(
          !useHumanModel &&
          ...
          // ----------------------------------------------------------
          // A. KataGo's actual NNOutput(ptrs) 8-sym ensemble.
          // averageMultipleSymmetries samples all 8 without replacement,
          // so order does not affect the arithmetic mean.
          // ----------------------------------------------------------
          {
            MiscNNInputParams multiParams = nnInputParams;

            NNResultBuf multiBuf;

            std::shared_ptr<NNOutput>* multiPtr =
              nnEvalToUse->averageMultipleSymmetries(
                board,
                hist,
                nextPla,
                &analysisParams.humanSLProfile,
                multiParams,
                multiBuf,
                true,
                gtpRand,
            ...
            testAssert(multiPtr != nullptr);
            testAssert((*multiPtr) != nullptr);

            const NNOutput* multiOut = multiPtr->get();

            out
              << "WG_MULTISYM8"
              << " has=" << (multiOut->hasWeakGroupDanger ? 1 : 0)
              << " current=" << Global::strprintf(
                   "%.9f",
                   multiOut->weakGroupCurrentDanger
                 )
              << " opponent=" << Global::strprintf(
                   "%.9f",
                   multiOut->weakGroupOpponentDanger
                 )
              << endl;
          ...
          // ----------------------------------------------------------
          // B. Ownership-only cache preservation.
          //
          // First:
          //   symmetry 0, no owner map, cache enabled
          //
          // Second:
          //   symmetry 7 requested, owner map requested, same cache key.
          //
          // Symmetry is deliberately not independently cached, which is
          // precisely why KataGo has the hadResultWithoutOwnerMap branch.
          // The second eval should compute ownership in the new orientation
          // but preserve policy/value/WG scalar from the cached first eval.
          // ----------------------------------------------------------
          {
            MiscNNInputParams cacheParamsA = nnInputParams;
            cacheParamsA.symmetry = 0;

            // Give this diagnostic a distinctive optimism value so it is
            // extremely unlikely to collide with an earlier normal rawNN
            // cache entry for this position.
            cacheParamsA.policyOptimism = 0.314159;

            NNResultBuf cacheBufA;

            bool cacheSkipA = false;
            bool ownerA = false;

            nnEvalToUse->evaluate(
              board,
              hist,
              nextPla,
              &analysisParams.humanSLProfile,
            ...
            testAssert(cacheBufA.result != nullptr);

            const bool firstHas =
              cacheBufA.result->hasWeakGroupDanger;
            const float firstCurrent =
              cacheBufA.result->weakGroupCurrentDanger;
            const float firstOpponent =
              cacheBufA.result->weakGroupOpponentDanger;

            MiscNNInputParams cacheParamsB = cacheParamsA;
            cacheParamsB.symmetry = 7;

            NNResultBuf cacheBufB;

            bool cacheSkipB = false;
            bool ownerB = true;
            ...
            testAssert(cacheBufB.result != nullptr);

            out
              << "WG_CACHE_OWNERMAP"
              << " firstHas=" << (firstHas ? 1 : 0)
              << " firstCurrent=" << Global::strprintf(
                   "%.9f",firstCurrent
                 )
              << " firstOpponent=" << Global::strprintf(
                   "%.9f",firstOpponent
                 )
              << " secondHas="
              << (cacheBufB.result->hasWeakGroupDanger ? 1 : 0)
              << " secondCurrent=" << Global::strprintf(
                   "%.9f",
                   cacheBufB.result->weakGroupCurrentDanger
                 )
              << " secondOpponent=" << Global::strprintf(
                   "%.9f",
                   cacheBufB.result->weakGroupOpponentDanger
                 )
              << " owner="
              << (cacheBufB.result->whiteOwnerMap != NULL ? 1 : 0)
              << endl;
          }
        }

        // PHASE-3B WG GROUP DIAGNOSTIC
        // buf.weakGroupMap is now:
        // canonical board coordinates,
        // CHW [current-player, opponent],
        // Scale8-restored and sigmoid-postprocessed probabilities.
        if(!useHumanModel && buf.hasWeakGroupMap) {
          const int wgArea =
            nnEvalToUse->getNNXLen() *
            nnEvalToUse->getNNYLen();

          testAssert(
            buf.weakGroupMap.size() ==
            (size_t)(2 * wgArea)
          );

          bool seenHeads[Board::MAX_ARR_SIZE] = {false};

          out << "WG_GROUPS nextPla="
              << PlayerIO::playerToStringShort(nextPla)
              << endl;
          ...
          for(int y = 0; y < board.y_size; y++) {
            for(int x = 0; x < board.x_size; x++) {
              ...
              if(color == nextPla) {
                ...
                testAssert(color == getOpp(nextPla));
                channel = 1;
                role = "OPPONENT";
              }
              ...
              do {
                ...
                testAssert(prob >= 0.0f && prob <= 1.0f);

                if(n > 0)
                  stones << ",";

                stones << Location::toString(cur,board);

                sum += prob;
                sumSq += (double)prob * (double)prob;
              ...
              out
                ...
                << " role=" << role
                << " ch=" << channel
                << " color=" << PlayerIO::playerToStringShort(color)
                << " head=" << Location::toString(head,board)
                << " size=" << n
                << " libs=" << board.getNumLiberties(head)
                << " mean=" << Global::strprintf("%.6f",mean)
                << " std=" << Global::strprintf("%.6f",stdev)
                << " min=" << Global::strprintf("%.6f",minProb)
                << " max=" << Global::strprintf("%.6f",maxProb)
                << " range=" << Global::strprintf("%.6f",maxProb-minProb)
                << " stones=" << stones.str()
                << endl;
            }
          }
