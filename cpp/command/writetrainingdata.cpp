...
static void parseSGFRank(
  ...
  try {
    ...
    //Fox uses forms such as P1段, P2段, ... P9段 for professional ranks.
    //Handle these before the generic Chinese dan parser below.
    else if(
      whatDataSource == "fox"
      && Global::isPrefix(rankStr,"P")
      && Global::isSuffix(rankStr,"\xE6\xAE\xB5")
    ) {
      string foxProRank = Global::chopSuffix(rankStr,"\xE6\xAE\xB5");
      rankNumber = Global::stringToInt(foxProRank.substr(1));
      isPro = true;
    }
    //UTF-8 for 段(dan)
  ...
  auto processSgf = [&](int threadIdx, size_t index) {
    ...
    if(whatDataSource == "ogs" || whatDataSource == "kgs") {
      ...
      if(sgfRules == "Japanese" && rules.komi == 0.0 && sgfHandicapParsed == 0) {
        ...
        || (sgfRules == "Japanese" && rules.komi == 5.5 && sgfHandicapParsed == 0)
    ...
    if(whatDataSource == "ogs" || whatDataSource == "kgs") {
      ...
      if(sgfTM != "" && sgfTC != "" && sgfTT != "") {
        ...
        // Older Fox SGFs may omit TM/TC/TT entirely.
        // Preserve the game and mark its time control as unknown.
        tcIsUnknown = true;
      }
    ...
    for(size_t m = startGameAt; m<sgfMoves.size()+1; m++) {
      ...
      if(nextPla != hist.presumedNextMovePla) {
        logger.write(
          "NEXTPLA_MISMATCH stage=sgf-replay file=" + fileName +
          " nextPla=" + Global::intToString((int)nextPla) +
          " histPresumed=" + Global::intToString((int)hist.presumedNextMovePla)
        );
        reportSgfDone(false,"NextPlaHistoryMismatch");
        return;
      }
    ...
    if(sgfGameEndedByScore && !rulesDisableValueTraining) {
      ...
      bool gameIsNearEnd = false;
      {
        ...
        {
          // Hypothetically evaluate this finished position with Black to move.
          // Search::setPosition does not synchronize BoardHistory::presumedNextMovePla,
          // so use a local history copy consistent with the hypothetical player.
          BoardHistory histIfBlackFirst(hist);
          histIfBlackFirst.presumedNextMovePla = P_BLACK;
          search->setPosition(P_BLACK,board,histIfBlackFirst);
        ...
        {
          // Same hypothetical evaluation with White to move.
          BoardHistory histIfWhiteFirst(hist);
          histIfWhiteFirst.presumedNextMovePla = P_WHITE;
          search->setPosition(P_WHITE,board,histIfWhiteFirst);
      ...
      if(!gameIsNearEnd) {
        ...
        for(int extraM = 0; extraM<maxKataFinishMoves; extraM++) {
          if(nextPla != hist.presumedNextMovePla) {
            logger.write(
              "NEXTPLA_MISMATCH stage=scored-cleanup-search file=" + fileName +
              " nextPla=" + Global::intToString((int)nextPla) +
              " histPresumed=" + Global::intToString((int)hist.presumedNextMovePla)
            );
            reportSgfDone(false,"NextPlaHistoryMismatch");
          ...
          if(nextPla != hist.presumedNextMovePla) {
            logger.write(
              "NEXTPLA_MISMATCH stage=scored-cleanup-value file=" + fileName +
              " nextPla=" + Global::intToString((int)nextPla) +
              " histPresumed=" + Global::intToString((int)hist.presumedNextMovePla)
            );
            reportSgfDone(false,"NextPlaHistoryMismatch");
        ...
        if(gameFinishedProperly) {
          ...
          bool weirdScoringState = false;
          {
            ...
            if(nextPla != hist.presumedNextMovePla) {
              logger.write(
                "NEXTPLA_MISMATCH stage=final-scoring-nneval file=" + fileName +
                " nextPla=" + Global::intToString((int)nextPla) +
                " histPresumed=" + Global::intToString((int)hist.presumedNextMovePla)
              );
              reportSgfDone(false,"NextPlaHistoryMismatch");
    ...
    for(size_t m = 0; m<policyTargets.size(); m++) {
      ...
      const int mode = 0;
      // Human SGF data has no search-generated Q-value targets.
      // Make one empty QValueTargets entry per turn so trainingwrite
      // can emit zero-filled Q-value target planes.
      const std::vector<QValueTargets> whiteQValueTargets(
        whiteValueTargets.size()
      );
