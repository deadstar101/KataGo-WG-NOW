...
// ============================================================
// WG Phase-5A OpenCL Dual-WG head
//
// Same frozen semantics as CUDA:
//   trunk -> conv1 -> bias1 + activation -> conv2 -> bias2
//
// OpenCL spatial tensors are NCHW.
// Device buffers may use padded XY and FP16 storage.
// Padding is masked here and cropped during host readback.
// Raw logits only - Scale8 restoration and sigmoid remain in nneval.cpp.
// ============================================================

struct WeakGroupSpatialBiasActLayer {
  const int numChannels;
  const int activation;
  const int paddedNNXYLen;
  ...
  WeakGroupSpatialBiasActLayer() = delete;
  WeakGroupSpatialBiasActLayer(const WeakGroupSpatialBiasActLayer&) = delete;
  WeakGroupSpatialBiasActLayer& operator=(const WeakGroupSpatialBiasActLayer&) = delete;

  WeakGroupSpatialBiasActLayer(
    ComputeHandleInternal* handle,
    ...
    biasBuf(NULL)
  {
    ...
    testAssert(desc->numChannels == numChannels);
    testAssert((int)desc->weights.size() == numChannels);

    vector<float> scales((size_t)numChannels,1.0f);
    vector<float> biases = desc->weights;

    scaleBuf = createReadOnlyBuffer(handle,scales,useFP16);
    biasBuf = createReadOnlyBuffer(handle,biases,useFP16);
  }

  ~WeakGroupSpatialBiasActLayer() {
    if(scaleBuf != NULL)
      clReleaseMemObject(scaleBuf);
    if(biasBuf != NULL)
      clReleaseMemObject(biasBuf);
  }
  ...
  void apply(
    ...
    if(activation == ACTIVATION_IDENTITY)
      ...
      assert(false);
      Global::fatalError("bad WG activation");
    }

    clSetKernelArg(kernel,0,sizeof(cl_mem),(const void*)&buf);
    clSetKernelArg(kernel,1,sizeof(cl_mem),(const void*)&buf);
    clSetKernelArg(kernel,2,sizeof(cl_mem),(const void*)&scaleBuf);
    clSetKernelArg(kernel,3,sizeof(cl_mem),(const void*)&biasBuf);
    clSetKernelArg(kernel,4,sizeof(cl_mem),(const void*)&mask);
    clSetKernelArg(kernel,5,sizeof(int),(const void*)&batchSize);
    clSetKernelArg(kernel,6,sizeof(int),(const void*)&numChannels);
    clSetKernelArg(kernel,7,sizeof(int),(const void*)&paddedNNXYLen);
    ...
    size_t globalSizes[nKernelDims] = {
      ...
      powerOf2ify((size_t)numChannels)
    };
    ...
    CHECK_ERR(err);
    MAYBE_PROFILE("WGSpatialBiasAct");
    MAYBE_FREE_EVENT;
  }
};


struct WeakGroupHead {
  const int midChannels;
  const int outChannels;
  const int nnXLen;
  const int nnYLen;
  const int paddedNNXYLen;

  std::unique_ptr<ConvLayer> conv1;
  std::unique_ptr<WeakGroupSpatialBiasActLayer> bias1;
  std::unique_ptr<ConvLayer> conv2;
  std::unique_ptr<WeakGroupSpatialBiasActLayer> bias2;

  WeakGroupHead() = delete;
  WeakGroupHead(const WeakGroupHead&) = delete;
  WeakGroupHead& operator=(const WeakGroupHead&) = delete;

  WeakGroupHead(
    ComputeHandleInternal* handle,
    const WeakGroupHeadDesc* desc,
    int nnX,
    int nnY,
    bool useFP16
  ) :
    midChannels(desc->conv1.outChannels),
    outChannels(desc->conv2.outChannels),
    nnXLen(nnX),
    nnYLen(nnY),
    paddedNNXYLen(handle->paddedNNXYLen)
  {
    testAssert(outChannels == 2);
    testAssert(desc->bias1.numChannels == midChannels);
    testAssert(desc->bias2.numChannels == outChannels);

    conv1 = std::make_unique<ConvLayer>(
      handle,&desc->conv1,nnXLen,nnYLen,useFP16
    );

    bias1 = std::make_unique<WeakGroupSpatialBiasActLayer>(
      handle,&desc->bias1,midChannels,desc->act1.activation,useFP16
    );

    conv2 = std::make_unique<ConvLayer>(
      handle,&desc->conv2,nnXLen,nnYLen,useFP16
    );

    bias2 = std::make_unique<WeakGroupSpatialBiasActLayer>(
      handle,&desc->bias2,outChannels,ACTIVATION_IDENTITY,useFP16
    );
  }
  ...
  ConvWorkspaceEltsNeeded requiredConvWorkspaceElts(
    ...
    maxElts = ConvWorkspaceEltsNeeded::getMax(
      maxElts,
      conv1->requiredConvWorkspaceElts(handle,maxBatchSize)
    );
    maxElts = ConvWorkspaceEltsNeeded::getMax(
      maxElts,
      conv2->requiredConvWorkspaceElts(handle,maxBatchSize)
    );
    return maxElts;
  }
  ...
  void apply(
    ...
    SizedBuf<cl_mem> mid(
      scratch->allocator,
      scratch->getBufSizeXY(midChannels)
    );
...
struct Model {
  ...
  bool hasWeakGroupHead;
  std::unique_ptr<WeakGroupHead> weakGroupHead;
  ...
  Model(
    ...
    hasWeakGroupHead = desc->hasWeakGroupHead;
    if(hasWeakGroupHead) {
      testAssert(desc->weakGroupHead.conv2.outChannels == 2);
      weakGroupHead = std::make_unique<WeakGroupHead>(
        handle,&desc->weakGroupHead,nnXLen,nnYLen,useFP16
      );
    }
    else
      weakGroupHead.reset();
  ...
  ConvWorkspaceEltsNeeded requiredConvWorkspaceElts(ComputeHandleInternal* handle) const {
    ...
    if(hasWeakGroupHead) {
      maxElts = ConvWorkspaceEltsNeeded::getMax(
        maxElts,
        weakGroupHead->requiredConvWorkspaceElts(handle,maxBatchSize)
      );
    }
    return maxElts;
  }
...
struct Buffers {
  ...
  Buffers(ComputeHandleInternal* handle, const Model& m) {
    ...
    if(m.hasWeakGroupHead) {
      testAssert(m.weakGroupHead != nullptr);
      weakGroupElts = m.weakGroupHead->outChannels * batchXYElts;
      weakGroup = createReadWriteBuffer(handle,weakGroupElts,useFP16);
    }
    else {
      weakGroupElts = 0;
      weakGroup = NULL;
...
struct InputBuffers {
  ...
  size_t singleWeakGroupResultElts;
  ...
  size_t weakGroupResultBufferElts;
  ...
  float* weakGroupResults; //Host pointer, canonical unpadded NCHW
  half_t* weakGroupResultsHalf; //Host pointer, padded NCHW
  ...
  InputBuffers(const LoadedModel* loadedModel, int maxBatchSz, int nnXLen, int nnYLen) {
    ...
    singleWeakGroupResultElts =
      m.hasWeakGroupHead ?
      (size_t)m.weakGroupHead.conv2.outChannels * nnXLen * nnYLen :
      0;
    ...
    weakGroupResultBufferElts =
      m.hasWeakGroupHead ?
      (size_t)maxBatchSize * m.weakGroupHead.conv2.outChannels * nnXLen * nnYLen :
      0;
    ...
    if(m.hasWeakGroupHead) {
      const int wgChannels = m.weakGroupHead.conv2.outChannels;
      testAssert(wgChannels == 2);
      weakGroupResults =
        new float[(size_t)maxBatchSize * nnXLen * nnYLen * wgChannels];
      weakGroupResultsHalf =
        new half_t[(size_t)maxBatchSize * maxPaddedNNXYLen * wgChannels];
    }
    else {
      weakGroupResults = NULL;
      weakGroupResultsHalf = NULL;
    }
  }
  ...
  ~InputBuffers() {
    ...
    delete[] weakGroupResults;
    delete[] weakGroupResultsHalf;
  }
...
void NeuralNet::getOutput(
  ...
  // WG Phase-5A OpenCL readback.
  // OpenCL spatial layout is always NCHW.
  // Keep RAW logits here; restoration/sigmoid happen in nneval.cpp.
  if(gpuHandle->model->hasWeakGroupHead) {
    testAssert(gpuHandle->model->weakGroupHead != nullptr);
    const int wgChannels = gpuHandle->model->weakGroupHead->outChannels;
    testAssert(wgChannels == 2);
    testAssert(inputBuffers->singleWeakGroupResultElts == (size_t)(wgChannels * nnXYLen));

    size_t paddedWeakGroupElts =
      (size_t)wgChannels * paddedNNXYLen * batchSize;
    ...
    if(useFP16Storage) {
      err = clEnqueueReadBuffer(
        ...
        blocking,
        0,
        paddedWeakGroupElts * sizeof(half_t),
        inputBuffers->weakGroupResultsHalf,
        0,
        NULL,
        NULL
      );
      ...
      size_t totalChannels = (size_t)wgChannels * batchSize;

      if(paddedNNXYLen == nnXYLen) {
        for(size_t i = 0; i < totalChannels * nnXYLen; i++)
          inputBuffers->weakGroupResults[i] =
            inputBuffers->weakGroupResultsHalf[i];
      }
      else {
        for(size_t c = 0; c < totalChannels; c++) {
          for(int xy = 0; xy < nnXYLen; xy++) {
            inputBuffers->weakGroupResults[c * nnXYLen + xy] =
              inputBuffers->weakGroupResultsHalf[c * paddedNNXYLen + xy];
          }
        }
      }
    }
    else {
      testAssert(paddedNNXYLen == nnXYLen);
      err = clEnqueueReadBuffer(
        handle->commandQueue,
        buffers->weakGroup,
        blocking,
        0,
        inputBuffers->singleWeakGroupResultElts * sizeof(float) * batchSize,
        inputBuffers->weakGroupResults,
        0,
        NULL,
        NULL
      );
  ...
  for(int row = 0; row < batchSize; row++) {
    ...
    // WG Phase-5A:
    // OpenCL is NCHW already, so unlike CUDA NHWC no deinterleave is needed.
    // Canonical result is CHW [current H*W][opponent H*W].
    if(gpuHandle->model->hasWeakGroupHead) {
      testAssert(gpuHandle->model->weakGroupHead != nullptr);
      testAssert(gpuHandle->model->weakGroupHead->outChannels == 2);
      testAssert(inputBuffers->singleWeakGroupResultElts == (size_t)(2 * nnXLen * nnYLen));

      NNResultBuf* resultBuf = inputBufs[row];
      resultBuf->weakGroupMap.resize((size_t)(2 * nnXLen * nnYLen));

      const float* weakGroupSrcBuf =
        inputBuffers->weakGroupResults +
        row * inputBuffers->singleWeakGroupResultElts;

      SymmetryHelpers::copyOutputsWithSymmetry(
        weakGroupSrcBuf,
        resultBuf->weakGroupMap.data(),
        2,
        nnYLen,
        nnXLen,
      ...
      resultBuf->hasWeakGroupMap = true;
    }
    else {
      inputBufs[row]->hasWeakGroupMap = false;
      inputBufs[row]->weakGroupMap.clear();
    }
