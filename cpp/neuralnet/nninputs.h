...
struct NNOutput {
  ...
  // These are relative to the player to move for this NN evaluation.
  // The full 2xHxW WG map remains transient in NNResultBuf and is NOT cached.
  bool hasWeakGroupDanger;
  float weakGroupCurrentDanger;
  float weakGroupOpponentDanger;

  // WG_REPLY after the player-to-move's predicted Top1 move.
  // Perspective remains PRE-MOVE current player / opponent.
  bool hasWeakGroupReplyDanger;
  float weakGroupReplyCurrentDanger;
  float weakGroupReplyOpponentDanger;
