...
WeakGroupHeadDesc::WeakGroupHeadDesc()
  : modelVersion(-1)
{}

WeakGroupHeadDesc::WeakGroupHeadDesc(
  istream& in, int vrsn, bool binaryFloats
) {
  in >> name;
  modelVersion = vrsn;

  if(in.fail())
    throw StringError("weak group head failed to parse name");

  if(modelVersion < 17)
    throw StringError(name + ": weak group head requires model version >= 17");

  conv1 = ConvLayerDesc(in,binaryFloats);
  bias1 = MatBiasLayerDesc(in,binaryFloats);
  act1 = ActivationLayerDesc(in,modelVersion);
  conv2 = ConvLayerDesc(in,binaryFloats);
  bias2 = MatBiasLayerDesc(in,binaryFloats);

  if(in.fail())
    throw StringError(name + ": weak group head istream fail after parsing layers");

  if(conv1.convXSize != 1 || conv1.convYSize != 1)
    throw StringError(
      name + Global::strprintf(
        ": conv1 must be 1x1, got %dx%d",
        conv1.convXSize, conv1.convYSize));

  if(conv2.convXSize != 1 || conv2.convYSize != 1)
    throw StringError(
      name + Global::strprintf(
        ": conv2 must be 1x1, got %dx%d",
        conv2.convXSize, conv2.convYSize));

  if(conv1.outChannels != bias1.numChannels)
    throw StringError(
      name + Global::strprintf(
        ": conv1.outChannels (%d) != bias1.numChannels (%d)",
        conv1.outChannels, bias1.numChannels));

  if(conv1.outChannels != conv2.inChannels)
    throw StringError(
      name + Global::strprintf(
        ": conv1.outChannels (%d) != conv2.inChannels (%d)",
        conv1.outChannels, conv2.inChannels));

  if(conv2.outChannels != bias2.numChannels)
    throw StringError(
      name + Global::strprintf(
        ": conv2.outChannels (%d) != bias2.numChannels (%d)",
        conv2.outChannels, bias2.numChannels));

  if(conv2.outChannels != 2)
    throw StringError(
      name + Global::strprintf(
        ": Dual-WG requires conv2.outChannels == 2, got %d",
        conv2.outChannels));

  if(act1.activation != ACTIVATION_RELU)
    throw StringError(name + ": Dual-WG act1 must be ACTIVATION_RELU");
}

WeakGroupHeadDesc::~WeakGroupHeadDesc() {}

WeakGroupHeadDesc::WeakGroupHeadDesc(WeakGroupHeadDesc&& other) {
  *this = std::move(other);
}

WeakGroupHeadDesc& WeakGroupHeadDesc::operator=(WeakGroupHeadDesc&& other) {
  name = std::move(other.name);
  modelVersion = other.modelVersion;
  conv1 = std::move(other.conv1);
  bias1 = std::move(other.bias1);
  act1 = std::move(other.act1);
  conv2 = std::move(other.conv2);
  bias2 = std::move(other.bias2);
  return *this;
}

void WeakGroupHeadDesc::iterConvLayers(
  const std::function<void(const ConvLayerDesc& desc)>& f
) const {
  f(conv1);
  f(conv2);
}

int64_t WeakGroupHeadDesc::getNumParameters() const {
  return
    conv1.getNumParameters() +
    bias1.getNumParameters() +
    conv2.getNumParameters() +
    bias2.getNumParameters();
}

void WeakGroupHeadDesc::transformToReduceActivations() {
  // Nothing to fold: this head has no normalization layer.
}

void WeakGroupHeadDesc::applyScale8ToReduceActivations() {
  // The incoming trunk tensor has been scaled by 1/8.
  // Scaling both biases by 1/8 makes:
  //
  // conv1+bias1 -> ReLU -> conv2+bias2
  //
  // exactly 1/8 of the original WG logits. The common
  // postProcessParams.outputScaleMultiplier restores x8 at decode time.
  bias1.applyScale8ToReduceActivations();
  act1.applyScale8ToReduceActivations();
  bias2.applyScale8ToReduceActivations();
}

void WeakGroupHeadDesc::releaseWeights() {
  conv1.releaseWeights();
  bias1.releaseWeights();
  conv2.releaseWeights();
  bias2.releaseWeights();
}
...
ModelDesc::ModelDesc()
  : modelVersion(-1),
    ...
    postProcessParams(),
      hasWeakGroupHead(false),
      weakGroupHead(),
      hasWeakGroupReplyHead(false),
      weakGroupReplyHead(),
    archSummary()
{}
...
ModelDesc::ModelDesc(istream& in, const string& sha256_, bool binaryFloats) {
  ...
  if(modelVersion >= 15) {
    ...
    int weakGroupOptionD = 0;
    in >> weakGroupOptionD;
    if(weakGroupOptionD == 0)
      hasWeakGroupHead = false;
    else if(weakGroupOptionD == 1 && modelVersion >= 17)
      hasWeakGroupHead = true;
    else
      throw StringError(
        name + ": unknown/unsupported model option D: " +
        Global::intToString(weakGroupOptionD));

    int weakGroupReplyOptionE = 0;
    in >> weakGroupReplyOptionE;
    if(weakGroupReplyOptionE == 0)
      hasWeakGroupReplyHead = false;
    else if(weakGroupReplyOptionE == 1 && modelVersion >= 17)
      hasWeakGroupReplyHead = true;
    else
      throw StringError(
        name + ": unknown/unsupported model option E: " +
        Global::intToString(weakGroupReplyOptionE));
  ...
  if(hasWeakGroupHead)
    weakGroupHead = WeakGroupHeadDesc(in, modelVersion, binaryFloats);
  if(hasWeakGroupReplyHead)
    weakGroupReplyHead = WeakGroupHeadDesc(in, modelVersion, binaryFloats);
  ...
  if(hasWeakGroupHead && trunk.trunkNumChannels != weakGroupHead.conv1.inChannels)
    throw StringError(
      name + Global::strprintf(
               ": trunk.trunkNumChannels (%d) != weakGroupHead.conv1.inChannels (%d)",
               trunk.trunkNumChannels,
               weakGroupHead.conv1.inChannels));

  if(hasWeakGroupReplyHead && trunk.trunkNumChannels != weakGroupReplyHead.conv1.inChannels)
    throw StringError(
      name + Global::strprintf(
               ": trunk.trunkNumChannels (%d) != weakGroupReplyHead.conv1.inChannels (%d)",
               trunk.trunkNumChannels,
               weakGroupReplyHead.conv1.inChannels));
}
...
ModelDesc& ModelDesc::operator=(ModelDesc&& other) {
  ...
  hasWeakGroupHead = other.hasWeakGroupHead;
  hasWeakGroupReplyHead = other.hasWeakGroupReplyHead;
  ...
  weakGroupHead = std::move(other.weakGroupHead);
  weakGroupReplyHead = std::move(other.weakGroupReplyHead);
  return *this;
}
...
void ModelDesc::iterConvLayers(const std::function<void(const ConvLayerDesc& desc)>& f) const {
  ...
  if(hasWeakGroupHead)
    weakGroupHead.iterConvLayers(f);
  if(hasWeakGroupReplyHead)
    weakGroupReplyHead.iterConvLayers(f);
}
...
int64_t ModelDesc::getNumParameters() const {
  ...
  return
    trunk.getNumParameters() +
    policyHead.getNumParameters() +
    valueHead.getNumParameters() +
    (hasWeakGroupHead ? weakGroupHead.getNumParameters() : 0) +
    (hasWeakGroupReplyHead ? weakGroupReplyHead.getNumParameters() : 0);
}
...
void ModelDesc::transformToReduceActivations() {
  ...
  if(hasWeakGroupHead)
    weakGroupHead.transformToReduceActivations();
  if(hasWeakGroupReplyHead)
    weakGroupReplyHead.transformToReduceActivations();
}
...
bool ModelDesc::applyScale8ToReduceActivations() {
  ...
  if(hasWeakGroupHead)
    weakGroupHead.applyScale8ToReduceActivations();
  if(hasWeakGroupReplyHead)
    weakGroupReplyHead.applyScale8ToReduceActivations();
...
void ModelDesc::releaseWeights() {
  ...
  if(hasWeakGroupHead)
    weakGroupHead.releaseWeights();
  if(hasWeakGroupReplyHead)
    weakGroupReplyHead.releaseWeights();
}
