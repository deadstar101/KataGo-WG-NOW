...
struct NNResultBuf {
  ...
  // WG Phase-3A transient buffer.
  // If present, layout is canonical CHW:
  // [current-player H*W][opponent H*W].
  // Values are RAW runtime logits after inverse symmetry but BEFORE
  // postProcessParams.outputScaleMultiplier and sigmoid.
  std::vector<float> weakGroupMap;
  bool hasWeakGroupMap;

  // WG_REPLY transient raw-logit map, canonical CHW [2,H,W].
  std::vector<float> weakGroupReplyMap;
  bool hasWeakGroupReplyMap;
