...
{
  // WG Phase-3C safe defaults for models without the WG head.
  hasWeakGroupDanger = false;
  weakGroupCurrentDanger = 0.0f;
  weakGroupOpponentDanger = 0.0f;
  hasWeakGroupReplyDanger = false;
  weakGroupReplyCurrentDanger = 0.0f;
  weakGroupReplyOpponentDanger = 0.0f;
}
NNOutput::NNOutput(const NNOutput& other) {
  ...
  hasWeakGroupDanger = other.hasWeakGroupDanger;
  weakGroupCurrentDanger = other.weakGroupCurrentDanger;
  weakGroupOpponentDanger = other.weakGroupOpponentDanger;
  hasWeakGroupReplyDanger = other.hasWeakGroupReplyDanger;
  weakGroupReplyCurrentDanger = other.weakGroupReplyCurrentDanger;
  weakGroupReplyOpponentDanger = other.weakGroupReplyOpponentDanger;
...
NNOutput::NNOutput(const vector<shared_ptr<NNOutput>>& others) {
  ...
  // WG Phase-3C:
  // each symmetry is reduced to node scalars first, then those
  // scalars are averaged by the normal NNOutput ensemble.
  hasWeakGroupDanger = others[0]->hasWeakGroupDanger;
  weakGroupCurrentDanger = 0.0f;
  weakGroupOpponentDanger = 0.0f;

  for(int i = 0; i<len; i++) {
    testAssert(others[i]->hasWeakGroupDanger == hasWeakGroupDanger);
    if(hasWeakGroupDanger) {
      weakGroupCurrentDanger += others[i]->weakGroupCurrentDanger;
      weakGroupOpponentDanger += others[i]->weakGroupOpponentDanger;
    }
  }

  if(hasWeakGroupDanger) {
    weakGroupCurrentDanger /= floatLen;
    weakGroupOpponentDanger /= floatLen;
  }

  // WG_REPLY scalars are symmetry-ensembled exactly like WG_NOW.
  hasWeakGroupReplyDanger = others[0]->hasWeakGroupReplyDanger;
  weakGroupReplyCurrentDanger = 0.0f;
  weakGroupReplyOpponentDanger = 0.0f;
  for(int i = 0; i<len; i++) {
    testAssert(others[i]->hasWeakGroupReplyDanger == hasWeakGroupReplyDanger);
    if(hasWeakGroupReplyDanger) {
      weakGroupReplyCurrentDanger += others[i]->weakGroupReplyCurrentDanger;
      weakGroupReplyOpponentDanger += others[i]->weakGroupReplyOpponentDanger;
    }
  }
  if(hasWeakGroupReplyDanger) {
    weakGroupReplyCurrentDanger /= floatLen;
    weakGroupReplyOpponentDanger /= floatLen;
  }
