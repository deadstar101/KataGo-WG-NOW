...
struct TrainingWriteBuffers {
  ...
  // WG3-V0: weak groups after the current player's top-1 policy response.
  // Shape [N,2,H,W], keeping the ORIGINAL position perspective:
  // C0 = original player-to-move, C1 = original opponent.
  NumpyBuffer<uint8_t> weakGroupReplyTargetsNCHW;
  NumpyBuffer<uint8_t> weakGroupReplyWeightNCHW;
  // Diagnostic only: exact post-response chain liberties, capped at 255.
  NumpyBuffer<uint8_t> weakGroupReplyLibertiesNCHW;

// WG_REPLY V1 raw supervision material.
// Input C0=P2 own, C1=P2 opponent, C2=P2 ladder plane14.
// Future C0=P2+8, C1=P2+32, C2=final ownership.
// Future values are {-1,0,+1}, from ORIGINAL nextPlayer perspective.
// Valid=1 only when both policy top-1 moves equal the real trajectory.
NumpyBuffer<uint8_t> weakGroupReplyV1InputNCHW;
NumpyBuffer<int8_t> weakGroupReplyV1FutureNCHW;
NumpyBuffer<uint8_t> weakGroupReplyV1ValidN;
