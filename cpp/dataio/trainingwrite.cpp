...
static const int WG_REPLY_TARGET_NUM_CHANNELS = 2;
static const int WG_REPLY_V1_INPUT_NUM_CHANNELS = 3;
static const int WG_REPLY_V1_FUTURE_NUM_CHANNELS = 3;
...
TrainingWriteBuffers::TrainingWriteBuffers(int iVersion, int maxRws, int numBChannels, int numFChannels, int xLen, int yLen, bool includeMetadata)
  ...
  weakGroupReplyTargetsNCHW({maxRws, WG_REPLY_TARGET_NUM_CHANNELS, yLen, xLen}),
  weakGroupReplyWeightNCHW({maxRws, WG_REPLY_TARGET_NUM_CHANNELS, yLen, xLen}),
  weakGroupReplyLibertiesNCHW({maxRws, WG_REPLY_TARGET_NUM_CHANNELS, yLen, xLen}),
weakGroupReplyV1InputNCHW({maxRws, WG_REPLY_V1_INPUT_NUM_CHANNELS, yLen, xLen}),
weakGroupReplyV1FutureNCHW({maxRws, WG_REPLY_V1_FUTURE_NUM_CHANNELS, yLen, xLen}),
weakGroupReplyV1ValidN({maxRws, 1}),
...
// WG_REPLY shared helper -----------------------------------------------
static bool getTopLegalPolicyMove(
  const Board& board,
  const BoardHistory& hist,
  Player pla,
  const vector<PolicyTargetMove>* policyTarget,
  Loc& bestLoc
) {
  if(policyTarget == NULL || policyTarget->empty())
    return false;

  bool found = false;
  bestLoc = Board::NULL_LOC;
  int bestPolicyTarget = -2147483647;

  for(size_t i = 0; i < policyTarget->size(); i++) {
    const PolicyTargetMove& move = (*policyTarget)[i];
    const int p = (int)move.policyTarget;

    if(p > bestPolicyTarget && hist.isLegal(board,move.loc,pla)) {
      bestPolicyTarget = p;
      bestLoc = move.loc;
      found = true;
    }
...
// WG3-V0 ---------------------------------------------------------------
// Simulate the current player's top-1 legal policy move, then classify
// groups on the resulting board.
//
// This intentionally does NOT reuse future8/future32/final ownership,
// since those targets belong to the actual game trajectory rather than
// this hypothetical top-1 branch.
//
// Channel perspective remains fixed to the input position:
//   C0 = input nextPlayer
//   C1 = opponent of input nextPlayer
static bool fillWeakGroupReplyTargetsV0(
  const Board& board,
  const BoardHistory& hist,
  Player nextPlayer,
  const vector<PolicyTargetMove>* policyTarget0,
  int dataXLen,
  int dataYLen,
  uint8_t* target,
  uint8_t* weight,
  ...
  const int posArea = dataXLen * dataYLen;
  const int total = WG_REPLY_TARGET_NUM_CHANNELS * posArea;
  ...
  if(policyTarget0 == NULL || policyTarget0->empty())
    return false;
  ...
  // PolicyTargetMove already stores Loc directly.
  for(size_t i = 0; i < policyTarget0->size(); i++) {
    const PolicyTargetMove& move = (*policyTarget0)[i];
    const int p = (int)move.policyTarget;

    if(p > bestPolicyTarget && hist.isLegal(board,move.loc,nextPlayer)) {
      bestPolicyTarget = p;
      bestLoc = move.loc;
      found = true;
    }
  ...
  Board postBoard(board);
  BoardHistory postHist(hist);

  // Correctly handles pass, captures, ko, suicide rules and history.
  postHist.makeBoardMoveAssumeLegal(
    postBoard,
    bestLoc,
    nextPlayer,
    NULL
  ...
  const Player originalPla = nextPlayer;
  const Player originalOpp = getOpp(nextPlayer);

  for(int y = 0; y < postBoard.y_size; y++) {
    for(int x = 0; x < postBoard.x_size; x++) {
      const Loc loc = Location::getLoc(x,y,postBoard.x_size);
      const Color color = postBoard.colors[loc];

      int channel;
      if(color == originalPla)
        channel = 0;
      else if(color == originalOpp)
        channel = 1;
      else
        continue;

      const int pos = NNPos::locToPos(
        loc,
        postBoard.x_size,
        dataXLen,
        dataYLen
      );
      testAssert(pos >= 0 && pos < posArea);

      const int libs = postBoard.getNumLiberties(loc);
      const int idx = channel * posArea + pos;

      liberties[idx] = (uint8_t)(libs > 255 ? 255 : libs);

      if(libs <= 3) {
        // Explicit tactical zone - leave masked.
        target[idx] = 0;
        weight[idx] = 0;
      }
      else if(libs == 4) {
        // WG_REPLY V0.1:
        // still only four liberties after the player's top-1 response,
        // so treat this as persistent weakness.
        target[idx] = 1;
        weight[idx] = 1;
      }
      else if(libs <= 7) {
        // Ambiguous strategic zone. Do not force weak/safe supervision.
        target[idx] = 0;
        weight[idx] = 0;
      }
      else {
        // Preserve WG2's conservative safe threshold.
        target[idx] = 0;
        weight[idx] = 1;
      }
    }
...
// WG3-V0 reply-aware weak-group target.
{
  const int wgReplyArea = dataXLen * dataYLen;

  uint8_t* rowWGReplyTarget =
    weakGroupReplyTargetsNCHW.data
    + curRows * WG_REPLY_TARGET_NUM_CHANNELS * wgReplyArea;

  uint8_t* rowWGReplyWeight =
    weakGroupReplyWeightNCHW.data
    + curRows * WG_REPLY_TARGET_NUM_CHANNELS * wgReplyArea;

  uint8_t* rowWGReplyLiberties =
    weakGroupReplyLibertiesNCHW.data
    + curRows * WG_REPLY_TARGET_NUM_CHANNELS * wgReplyArea;

  fillWeakGroupReplyTargetsV0(
    board,
    hist,
    nextPlayer,
    policyTarget0,
...
// WG_REPLY V2 P0-anchored raw supervision ------------------------------
// P0 --actual0/top0--> P1 --actual1/top1--> P2
//
// We accept the row only when BOTH policy top-1 moves equal the actual
// trajectory. P2 therefore selects the on-trajectory reply branch, while
// supervision is anchored to P0 groups using P0+10/P0+34/final outcomes.
{
  const int area = dataXLen * dataYLen;

  uint8_t* rowV1Input =
    weakGroupReplyV1InputNCHW.data
    + curRows * WG_REPLY_V1_INPUT_NUM_CHANNELS * area;

  int8_t* rowV1Future =
    weakGroupReplyV1FutureNCHW.data
    + curRows * WG_REPLY_V1_FUTURE_NUM_CHANNELS * area;

  uint8_t* rowV1Valid =
    weakGroupReplyV1ValidN.data + curRows;

  for(int i = 0; i < WG_REPLY_V1_INPUT_NUM_CHANNELS * area; i++)
    rowV1Input[i] = 0;
  for(int i = 0; i < WG_REPLY_V1_FUTURE_NUM_CHANNELS * area; i++)
    rowV1Future[i] = 0;
  rowV1Valid[0] = 0;
  ...
  bool valid =
    ...
    policyTarget0 != NULL &&
    policyTarget1 != NULL &&
    posHistForFutureBoards != NULL &&
    finalOwnership != NULL &&
    !actualGameEndHist.isNoResult &&
    turnIdx >= 0 &&
    turnIdx + 1 < (int)actualGameEndHist.moveHistory.size();

  Board p2Board(board);
  BoardHistory p2Hist(hist);

  if(valid) {
    const Move actual0 = actualGameEndHist.moveHistory[turnIdx];
    const Move actual1 = actualGameEndHist.moveHistory[turnIdx+1];

    Loc top0 = Board::NULL_LOC;

    valid =
      actual0.pla == nextPlayer &&
      getTopLegalPolicyMove(
        board,hist,nextPlayer,policyTarget0,top0
      ) &&
      top0 == actual0.loc &&
      hist.isLegal(board,actual0.loc,actual0.pla);

    if(valid) {
      p2Hist.makeBoardMoveAssumeLegal(
        p2Board,
        actual0.loc,
        actual0.pla,
        NULL
      ...
      const Player replyPla = getOpp(nextPlayer);
      Loc top1 = Board::NULL_LOC;

      valid =
        actual1.pla == replyPla &&
        getTopLegalPolicyMove(
          p2Board,p2Hist,replyPla,policyTarget1,top1
        ) &&
        top1 == actual1.loc &&
        p2Hist.isLegal(p2Board,actual1.loc,actual1.pla);
  ...
  if(valid) {
    // WG_REPLY V2: the head is evaluated on P0, so the spatial anchor
    // must be the ORIGINAL P0 groups, not the groups after the two
    // Top-1 moves at P2.
    //
    // binaryInputNCHWUnpacked still contains the P0 V7 input that was
    // filled above for the main network input. Reuse exactly planes
    // 1/2/14, matching WG_NOW:
    //   C0 = P0 original nextPlayer stones
    //   C1 = P0 original opponent stones
    //   C2 = P0 ladder-threatened stones
    //
    // p2Board/p2Hist above remain necessary only for validating that
    // BOTH Top-1 moves match the actual trajectory.

    const float* p0Bin = binaryInputNCHWUnpacked;

    for(int i = 0; i < area; i++) {
      rowV1Input[0*area+i] =
        p0Bin[1*area+i] > 0.5f ? 1 : 0;

      rowV1Input[1*area+i] =
        p0Bin[2*area+i] > 0.5f ? 1 : 0;

      rowV1Input[2*area+i] =
        p0Bin[14*area+i] > 0.5f ? 1 : 0;
    }
    ...
    const int endIdx = (int)boards.size()-1;

    // boards[t] is P0.
    // P2 = t+2.
    // Therefore P2-relative +8/+32 = t+10/t+34.
    const Board& future8Board =
      boards[std::min(whiteValueTargetsIdx + 10,endIdx)];

    const Board& future32Board =
      boards[std::min(whiteValueTargetsIdx + 34,endIdx)];

    testAssert(
      future8Board.x_size == board.x_size &&
      future8Board.y_size == board.y_size
    );
    testAssert(
      future32Board.x_size == board.x_size &&
      future32Board.y_size == board.y_size
    );

    const Player opp = getOpp(nextPlayer);

    for(int y = 0; y < board.y_size; y++) {
      for(int x = 0; x < board.x_size; x++) {
        const int pos = NNPos::xyToPos(x,y,dataXLen);
        const Loc loc =
          Location::getLoc(x,y,board.x_size);

        const Color c8 = future8Board.colors[loc];
        const Color c32 = future32Board.colors[loc];

        rowV1Future[0*area+pos] =
          c8 == nextPlayer ? 1 :
          (c8 == opp ? -1 : 0);

        rowV1Future[1*area+pos] =
          c32 == nextPlayer ? 1 :
          (c32 == opp ? -1 : 0);

        rowV1Future[2*area+pos] =
          finalOwnership[loc] == nextPlayer ? 1 :
          (finalOwnership[loc] == opp ? -1 : 0);
      }
    }
...
void TrainingWriteBuffers::writeToZipFile(const string& fileName) {
  ...
  numBytes = weakGroupReplyTargetsNCHW.prepareHeaderWithNumRows(curRows);
  zipFile.writeBuffer("weakGroupReplyTargetsNCHW", weakGroupReplyTargetsNCHW.dataIncludingHeader, numBytes);

  numBytes = weakGroupReplyWeightNCHW.prepareHeaderWithNumRows(curRows);
  zipFile.writeBuffer("weakGroupReplyWeightNCHW", weakGroupReplyWeightNCHW.dataIncludingHeader, numBytes);

  numBytes = weakGroupReplyLibertiesNCHW.prepareHeaderWithNumRows(curRows);
  zipFile.writeBuffer("weakGroupReplyLibertiesNCHW", weakGroupReplyLibertiesNCHW.dataIncludingHeader, numBytes);

  numBytes = weakGroupReplyV1InputNCHW.prepareHeaderWithNumRows(curRows);
  zipFile.writeBuffer("weakGroupReplyV1InputNCHW", weakGroupReplyV1InputNCHW.dataIncludingHeader, numBytes);

  numBytes = weakGroupReplyV1FutureNCHW.prepareHeaderWithNumRows(curRows);
  zipFile.writeBuffer("weakGroupReplyV1FutureNCHW", weakGroupReplyV1FutureNCHW.dataIncludingHeader, numBytes);

  numBytes = weakGroupReplyV1ValidN.prepareHeaderWithNumRows(curRows);
  zipFile.writeBuffer("weakGroupReplyV1ValidN", weakGroupReplyV1ValidN.dataIncludingHeader, numBytes);
