...
double Search::getExploreSelectionValueOfChild(
  ...
  // WG Phase-4A visited-child gate.
  // Record this BEFORE virtual losses can alter childWeight.
  // FPU / fresh children must never receive WG bias.
  const bool hasVisitedUtility = (childVisits > 0 && childWeight > 0.0);
  ...
  bool wgApplicable = false;
  double wgMoveScore = 0.0;
  double wgBonus = 0.0;

  // WG Phase-4C selection-only bias.
  //
  // Full parent->child WG edge score is:
  //
  //   (parent.current - parent.opponent)
  // + (child.current  - child.opponent)
  //
  // The parent term is identical for all siblings, so selection uses:
  //
  //   WGSelectionScore =
  //       child.currentDanger
  //     - child.opponentDanger
  //
  // Only already-visited/evaluated children participate.
  // New/FPU children use neutral WG score = 0.
  if(hasVisitedUtility) {
    const NNOutput* parentNNOutput = parent.getNNOutput();
    const NNOutput* childNNOutput = child->getNNOutput();

    // WG3 effective weak-group score:
    //
    //   WG3 (D=1,E=1):
    //     use WG_REPLY only.
    //
    //   WG2 compatibility (D=1,E=0):
    //     fall back to WG_NOW.
    //
    // WG_REPLY(child) is anchored to child.nextPla and represents the
    // weak-group state after child.nextPla's predicted Top1 reply.
    const bool parentHasEffectiveWG =
      parentNNOutput != NULL &&
      (
        parentNNOutput->hasWeakGroupReplyDanger ||
        parentNNOutput->hasWeakGroupDanger
      );

    const bool childHasEffectiveWG =
      childNNOutput != NULL &&
      (
        childNNOutput->hasWeakGroupReplyDanger ||
        childNNOutput->hasWeakGroupDanger
      );

    if(parentHasEffectiveWG && childHasEffectiveWG) {
      wgApplicable = true;

      double currentDanger;
      double opponentDanger;

      if(
        childNNOutput->hasWeakGroupReplyDanger &&
        childNNOutput->hasWeakGroupDanger
      ) {
        const double nowCurrent =
          (double)childNNOutput->weakGroupCurrentDanger;
        const double nowOpponent =
          (double)childNNOutput->weakGroupOpponentDanger;
        const double replyCurrent =
          (double)childNNOutput->weakGroupReplyCurrentDanger;
        const double replyOpponent =
          (double)childNNOutput->weakGroupReplyOpponentDanger;

        const double dNow = nowCurrent - nowOpponent;
        const double dReply = replyCurrent - replyOpponent;

        if(searchParams.weakGroupReplyMode == 0) {
          // NOW-only.
          currentDanger = nowCurrent;
          opponentDanger = nowOpponent;
        }
        else if(searchParams.weakGroupReplyMode == 2) {
          // Persistence-gated WG_REPLY correction.
          // REPLY may strengthen NOW only when both support
          // the same weak-group imbalance direction.
          double dEffective = dNow;
          ...
          if(
            ...
            const double absNow =
              dNow >= 0.0 ? dNow : -dNow;
            const double absReply =
              dReply >= 0.0 ? dReply : -dReply;
            const double persistentMagnitude =
              absNow < absReply ? absNow : absReply;

            dEffective +=
              (dNow > 0.0 ? 1.0 : -1.0) *
              searchParams.weakGroupReplyAlpha *
              persistentMagnitude;
          }

          // Downstream code uses only currentDanger-opponentDanger.
          currentDanger = dEffective;
          opponentDanger = 0.0;
        }
        else if(searchParams.weakGroupReplyMode == 3) {
          // WG_REPLY V2 sparse-delta:
          // keep WG_NOW as baseline and add only NEW positive danger
          // revealed by the reply horizon, independently for each side.
          const double threshold =
            searchParams.weakGroupReplyDeltaThreshold;

          double deltaCurrent =
            replyCurrent - nowCurrent - threshold;
          double deltaOpponent =
            replyOpponent - nowOpponent - threshold;

          if(deltaCurrent < 0.0)
            deltaCurrent = 0.0;
          if(deltaOpponent < 0.0)
            deltaOpponent = 0.0;

          const double dEffective =
            dNow +
            searchParams.weakGroupReplyAlpha *
            (deltaCurrent - deltaOpponent);
          ...
          // mode=1: original normalized linear blend.
          const double w =
            searchParams.weakGroupReplyBlendWeight;

          currentDanger =
            (nowCurrent + w * replyCurrent) / (1.0 + w);
          opponentDanger =
            (nowOpponent + w * replyOpponent) / (1.0 + w);
        }
      }
      else if(childNNOutput->hasWeakGroupReplyDanger) {
        currentDanger =
          (double)childNNOutput->weakGroupReplyCurrentDanger;
        opponentDanger =
          (double)childNNOutput->weakGroupReplyOpponentDanger;
        ...
        // Backward compatibility for old WG2 D=1,E=0 models.
        currentDanger =
          (double)childNNOutput->weakGroupCurrentDanger;
        opponentDanger =
          (double)childNNOutput->weakGroupOpponentDanger;
      ...
      wgMoveScore =
        currentDanger
        - opponentDanger
        - weakGroupSiblingCenter;

      wgBonus =
        searchParams.weakGroupUtilityLambda * wgMoveScore;

      // childUtility remains WHITE-perspective.
      if(searchParams.weakGroupUtilityLambda != 0.0) {
        if(parent.nextPla == P_WHITE)
          childUtility += wgBonus;
        else
          childUtility -= wgBonus;
      }
    }
  ...
  const double selectionValueAfterWG =
    getExploreSelectionValue(
      exploreScaling,
      nnPolicyProb,
      childWeight,
      childUtility,
  ...
  bool isRoot) const
{
  ...
  // WG Phase-4D sibling mean center.
  //
  // raw_i =
  //   child.currentDanger - child.opponentDanger
  //
  // center =
  //   MEAN(raw_i over WG-applicable visited siblings)
  //
  // final score_i = raw_i - center
  //
  // New/FPU/eval-cache-only candidates remain WG-neutral.
  double weakGroupSiblingCenter = 0.0;
  int weakGroupSiblingCount = 0;

  if(searchParams.weakGroupUtilityLambda != 0.0) {
    const NNOutput* parentWGOutput = node.getNNOutput();

    const bool parentHasEffectiveWG =
      parentWGOutput != NULL &&
      (
        parentWGOutput->hasWeakGroupReplyDanger ||
        parentWGOutput->hasWeakGroupDanger
      );

    if(parentHasEffectiveWG) {
      double weakGroupSiblingSum = 0.0;
      ...
      for(int i = 0; i < childrenCapacity; i++) {
        ...
        int64_t childEdgeVisits =
          childPointer.getEdgeVisits();

        double childWeight;
        if(countEdgeVisit)
          childWeight =
            child->stats.getChildWeight(
              childEdgeVisits,
              childVisits
            );
        else
          childWeight =
            child->stats.weightSum.load(
        ...
        // WG3 effective weak-group score:
        // prefer reply-after-Top1, otherwise old WG_NOW fallback.
        if(
          childNNOutput->hasWeakGroupReplyDanger &&
          childNNOutput->hasWeakGroupDanger
        ) {
        ...
        else if(childNNOutput->hasWeakGroupDanger) {
          currentDanger =
            (double)childNNOutput->weakGroupCurrentDanger;
          opponentDanger =
            (double)childNNOutput->weakGroupOpponentDanger;
        ...
        weakGroupSiblingSum +=
          currentDanger - opponentDanger;

        weakGroupSiblingCount++;
      }

      if(weakGroupSiblingCount > 0) {
        weakGroupSiblingCenter =
          weakGroupSiblingSum /
          (double)weakGroupSiblingCount;
      }
    }
  }
