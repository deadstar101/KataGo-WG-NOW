...
bool Search::getPlaySelectionValues(
  ...
  if(&node == rootNode && numChildren > 0) {
    ...
    // WG Phase-4D sibling center for play-selection results.
    //
    // Keep retrospective root play-selection/pruning consistent with
    // the actual MCTS child-selection path.
    //
    // Eligibility and centering semantics are identical:
    //
    //   raw_i = child.currentDanger - child.opponentDanger
    //   center = MEAN(raw_i over WG-applicable visited siblings)
    //
    // lambda=0 skips the preprocessing entirely.
      ...
      if(parentHasEffectiveWG) {
        ...
        for(int i = 0; i < numChildren; i++) {
          ...
          const SearchNode* child =
            childPointer.getIfAllocated();

          if(child == NULL)
            break;

          int64_t childVisits =
            child->stats.visits.load(
