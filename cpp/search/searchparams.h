...
struct SearchParams {
  ...
  // WG Phase-4A: selection-only utility bias strength.
  // 0.0 means EXACTLY disabled.
  double weakGroupUtilityLambda;

  // WG3: Relative weight of WG_REPLY versus WG_NOW when both exist.
  // 0.0 = NOW-only, 1.0 = 1:1, 2.0 = 1:2 (WG3 V0.2 default).
  double weakGroupReplyBlendWeight;

  // WG3 search integration mode:
  // 0 = WG_NOW only
  // 1 = linear NOW/REPLY blend
  // 2 = persistence-gated REPLY correction
  // 3 = sparse positive REPLY-minus-NOW danger correction
  int weakGroupReplyMode;

  // Strength of the REPLY correction in modes 2 and 3.
  double weakGroupReplyAlpha;

  // Mode 3 only: minimum positive per-side REPLY-minus-NOW danger
  // required before the sparse delta contributes to search.
  double weakGroupReplyDeltaThreshold;
