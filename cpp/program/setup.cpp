...
vector<SearchParams> Setup::loadParams(
  ...
  for(int i = 0; i<numBots; i++) {
    ...
    // WG Phase-4A selection-only strength.
    if(cfg.contains("weakGroupUtilityLambda"+idxStr))
      params.weakGroupUtilityLambda = cfg.getDouble("weakGroupUtilityLambda"+idxStr, 0.0, 2.0);
    else if(cfg.contains("weakGroupUtilityLambda"))
      params.weakGroupUtilityLambda = cfg.getDouble("weakGroupUtilityLambda", 0.0, 2.0);
    else
      params.weakGroupUtilityLambda = 0.0;

    // WG3 reply blend:
    //   effective = (NOW + w * REPLY) / (1 + w)
    // w=0 NOW-only, w=1 1:1, w=2 1:2, w=3 1:3.
    if(cfg.contains("weakGroupReplyBlendWeight"+idxStr))
      params.weakGroupReplyBlendWeight =
        cfg.getDouble("weakGroupReplyBlendWeight"+idxStr, 0.0, 10.0);
    else if(cfg.contains("weakGroupReplyBlendWeight"))
      params.weakGroupReplyBlendWeight =
        cfg.getDouble("weakGroupReplyBlendWeight", 0.0, 10.0);
    else
      params.weakGroupReplyBlendWeight = 2.0;

    if(cfg.contains("weakGroupReplyMode"+idxStr))
      params.weakGroupReplyMode =
        cfg.getInt("weakGroupReplyMode"+idxStr, 0, 3);
    else if(cfg.contains("weakGroupReplyMode"))
      params.weakGroupReplyMode =
        cfg.getInt("weakGroupReplyMode", 0, 3);
    else
      params.weakGroupReplyMode = 1;

    if(cfg.contains("weakGroupReplyAlpha"+idxStr))
      params.weakGroupReplyAlpha =
        cfg.getDouble("weakGroupReplyAlpha"+idxStr, 0.0, 2.0);
    else if(cfg.contains("weakGroupReplyAlpha"))
      params.weakGroupReplyAlpha =
        cfg.getDouble("weakGroupReplyAlpha", 0.0, 2.0);
    else
      params.weakGroupReplyAlpha = 0.25;

    if(cfg.contains("weakGroupReplyDeltaThreshold"+idxStr))
      params.weakGroupReplyDeltaThreshold =
        cfg.getDouble("weakGroupReplyDeltaThreshold"+idxStr, 0.0, 1.0);
    else if(cfg.contains("weakGroupReplyDeltaThreshold"))
      params.weakGroupReplyDeltaThreshold =
        cfg.getDouble("weakGroupReplyDeltaThreshold", 0.0, 1.0);
    else
      params.weakGroupReplyDeltaThreshold = 0.0;
